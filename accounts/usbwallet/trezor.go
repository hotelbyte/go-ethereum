...
import (
	...
	"github.com/hotelbyte/go-hotelbyte/accounts/usbwallet/internal/trezor"
...
// or whether the Hotelbyte app was not started on it.
...
// Hotelbyte address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.HotelbyteAddress)
	if _, err := w.trezorExchange(&trezor.HotelbyteGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.HotelbyteSignTx{
	...
	response := new(trezor.HotelbyteTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.HotelbyteTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
