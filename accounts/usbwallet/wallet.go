...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Hotelbyte
...
type wallet struct {
	...
	deriveChain    hotelbyte.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Hotelbyte account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain hotelbyte.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
