## Go Hotelbyte Dashboard

The dashboard is a data visualizer integrated into ghbc, intended to collect and visualize useful information of an Hotelbyte node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-hotelbyte` repo), these need to be installed first:
...
Normally the dashboard assets are bundled into Ghbc via `go-bindata` to avoid external dependencies. Rebuilding Ghbc after each UI modification however is not feasible from a developer perspective. Instead, we can run `webpack` in watch mode to automatically rebundle the UI, and ask `ghbc` to use external assets to not rely on compiled resources:

```
$ (cd dashboard/assets && ./node_modules/.bin/webpack --watch)
$ ghbc --dashboard --dashboard.assets=dashboard/assets --vmodule=dashboard=5
```

To bundle up the final UI into Ghbc, run `go generate`:
