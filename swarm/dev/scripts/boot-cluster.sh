...
#
# The cluster contains a bootnode, a ghbc node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the ghbc and
# swarm nodes
start_bootnode() {
...
# start_ghbc_node starts a ghbc node with --datadir pointing at <base-dir>/ghbc
# and a single, unlocked account with password "ghbc"
start_ghbc_node() {
  local dir="${base_dir}/ghbc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get ghbc account address"
  fi
  ...
  start_node "ghbc" "${GHBC_IP}" "$(which ghbc)" ${args[@]}
}
...
create_account() {
  ...
  ghbc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
